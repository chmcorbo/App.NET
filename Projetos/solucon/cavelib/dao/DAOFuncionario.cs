    ...
    public class DAOFuncionario : DAOBase
    {
        ...
        private bool buscar(ClasseBase obj, int i)
        {
            ...
            try
            {
                command.Connection = MsSQL.getConexao();
                command.Connection.Open();
                command.CommandText = "SELECT ID,MATRICULA,NOME,DATA_ADMISSAO,DATA_DEMISSAO,NUM_CNH,CLASSE_CNH," +
                    "VENCTO_CNH,ID_FUNCAO FROM FUNCIONARIO ";
                if (i == 1)
                    command.CommandText = command.CommandText + " WHERE ID=" + ((Funcionario)obj).ID.ToString();
                else
                    command.CommandText = command.CommandText + " WHERE MATRICULA='" + ((Funcionario)obj).Matricula + "'";
                ...
                if (reader.Read())
                {
                    ...
                    if (!reader.IsDBNull(reader.GetOrdinal("DATA_ADMISSAO")))
                       ((Funcionario)obj).Data_admissao = Convert.ToDateTime(reader["DATA_ADMISSAO"]);
                    if (!reader.IsDBNull(reader.GetOrdinal("DATA_DEMISSAO")))
                    ...
                    if (!reader.IsDBNull(reader.GetOrdinal("VENCTO_CNH")))
        ...
        public bool buscarMatricula(ClasseBase obj)
        {
            return buscar(obj, 2);
        }
